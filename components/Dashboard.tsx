  ...
  return (
    <div className="space-y-6">
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
        className="bg-[#12121a] border border-[#1e1e2e] rounded-xl p-6 flex flex-col sm:flex-row items-start sm:items-center gap-5">
        <div className="w-16 h-16 rounded-2xl bg-gradient-to-br from-[#6366f1] to-[#8b5cf6] flex items-center justify-center text-2xl font-bold text-white shadow-lg shadow-[#6366f1]/30">
          {level}
        </div>
        <div className="flex-1">
          <h2 className="text-2xl font-bold text-white">Level {level} Developer</h2>
          ...
          <div className="mt-3">
            ...
            <p className="text-xs text-[#64748b] mt-1">{xpToNext} XP until next level</p>
          </div>
        </div>
      </motion.div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}
          ...
          <h3 className="font-semibold text-white mb-5 flex items-center gap-2"><BarChart2 size={16} className="text-[#6366f1]" />Category Progress</h3>
          <div className="space-y-4">
            {categoryProgress.map(({ name, completed, total }) => (
              <div key={name}>
                <div className="flex justify-between text-sm mb-1.5">
                  <span className="text-[#e2e8f0] font-medium capitalize">{name}</span>
                  <span className="text-[#64748b]">{completed}/{total}</span>
                </div>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}
          ...
          <h3 className="font-semibold text-white mb-5 flex items-center gap-2"><Star size={16} className="text-[#f59e0b]" />By Difficulty</h3>
              ...
              return (
                <div key={label} className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs text-[#64748b] w-8 text-right">{done}/{total}</span>
                  </div>
                </div>
              )
            })}
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }}
        className="bg-[#12121a] border border-[#1e1e2e] rounded-xl p-6">
        <h3 className="font-semibold text-white mb-4 flex items-center gap-2"><Trophy size={16} className="text-[#f59e0b]" />Badges ({earnedBadges.length}/{badges.length})</h3>
        <div className="flex flex-wrap gap-3">
          {badges.map(badge => {
            const earned = earnedBadges.some(b => b.id === badge.id)
            return (
              <div key={badge.id} className={`flex items-center gap-2 px-3 py-2 rounded-lg border text-sm ${
                earned ? 'bg-[#6366f1]/10 border-[#6366f1]/40 text-white' : 'bg-[#1e1e2e]/40 border-[#1e1e2e] text-[#64748b] opacity-50'
              }`}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }}
          className="bg-[#12121a] border border-[#1e1e2e] rounded-xl p-6">
          <h3 className="font-semibold text-white mb-4 flex items-center gap-2"><Activity size={16} className="text-[#06b6d4]" />Completed Challenges</h3>
          {completedChallenges.length === 0
            ? <p className="text-[#64748b] text-sm">No challenges completed yet. Head to the Challenges page to get started!</p>
            : (
              <div className="space-y-2">
                {completedChallenges.slice().reverse().slice(0, 6).map((ch) => ch && (
                  <div key={ch.id} className="flex items-center justify-between py-2 border-b border-[#1e1e2e]/50 last:border-0">
                    <div className="flex items-center gap-2">
                      <CheckCircle size={14} className="text-[#10b981]" />
                      <span className="text-sm text-[#e2e8f0]">{ch.title}</span>
                    </div>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5 }}
          className="bg-[#12121a] border border-[#1e1e2e] rounded-xl p-6">
          <h3 className="font-semibold text-white mb-4 flex items-center gap-2"><Target size={16} className="text-[#8b5cf6]" />Recommended Next</h3>
              ...
              <div className="space-y-3">
                {recommended.map(ch => (
                  <div key={ch.id} className="flex items-center justify-between">
                    <div>
                      <p className="text-sm font-medium text-[#e2e8f0]">{ch.title}</p>
                      <p className="text-xs text-[#64748b]">{ch.difficulty} · {ch.category}</p>
                    </div>

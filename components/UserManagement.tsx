    ...
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-[#64748b] text-sm">{filtered.length} users found</p>
        </div>
      </div>

      {/* Filters */}
      <div className="bg-[#12121a] border border-[#1e1e2e] rounded-xl p-4 flex flex-wrap gap-3 items-center">
        <div className="flex-1 min-w-[200px] relative">
          <Search size={16} className="absolute left-3 top-1/2 -translate-y-1/2 text-[#64748b]" />
          <input
            className="w-full bg-[#0a0a0f] border border-[#1e1e2e] rounded-lg pl-9 pr-3 py-2 text-sm text-[#e2e8f0] placeholder-[#334155] focus:outline-none focus:border-[#6366f1]"
        ...
        <div className="flex items-center gap-2">
          ...
          <select
            className="bg-[#0a0a0f] border border-[#1e1e2e] rounded-lg px-3 py-2 text-sm text-[#e2e8f0] focus:outline-none focus:border-[#6366f1]"
            value={roleFilter}
            ...
            <thead>
              <tr className="border-b border-[#1e1e2e] bg-[#0a0a0f]/50">
                {([['name','Name'],['email','Email'],['role','Role'],['experiments','Experiments'],['joinDate','Joined'],['lastActive','Last Active'],['status','Status']] as [keyof User, string][]).map(([field, label]) => (
                  <th key={field} className="text-left px-4 py-3 text-[#64748b] font-medium cursor-pointer hover:text-white select-none" onClick={() => handleSort(field)}>
            ...
            <tbody>
              {paginated.map((user, i) => (
                <motion.tr key={user.id} initial={{ opacity: 0, y: 4 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.03 }}
                  ...
                  <td className="px-4 py-3 text-[#e2e8f0]">{user.experiments}</td>
                  <td className="px-4 py-3 text-[#64748b]">{user.joinDate}</td>
                  <td className="px-4 py-3 text-[#64748b]">{user.lastActive}</td>
                  <td className="px-4 py-3">
                    <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${
                      user.status === 'active' ? 'bg-[#10b981]/20 text-[#10b981] border border-[#10b981]/30' : 'bg-[#334155]/30 text-[#64748b] border border-[#334155]/30'
                    }`}>{user.status}</span>
                  </td>
                </motion.tr>
              ))}
          ...
          <div className="flex gap-1">
            {Array.from({ length: totalPages }, (_, i) => i + 1).slice(Math.max(0, page-3), page+2).map(p => (
              <button key={p} onClick={() => setPage(p)}
                className={`w-8 h-8 rounded-lg text-sm font-medium ${p === page ? 'bg-[#6366f1] text-white' : 'text-[#64748b] hover:bg-[#1e1e2e] hover:text-white'}`}
              >{p}</button>
            ))}
          </div>
        </div>
